...
ENT.PrintName = "Coal Vein"
ENT.RespawnTime = 3600 * 2
...
if SERVER then
	...
	function ENT:Initialize()
		...
		-- Даём большой запас «здоровья», чтобы срабатывал OnTakeDamage (руду считаем сами).
			...
			if string.find(activeWeapon, "tfa_nmrih_pickaxe") or string.find(activeWeapon, "wm_pickaxe") then
				...
				if self.depletedType < 4 then
					...
					if math.random(1, 100) <= chance then
						...
						local instance = ix.Item:Instance("mat_charcoal")

						if !self:AddStack(client, instance, 1) then
							ix.Item:Spawn(client, nil, instance)
						end

						client:RewardXP(10, "xp.gathering")
					end
				else
	...
	net.Receive("pickaxe.fx", function(_, client)
		local pos = net.ReadVector()
		local normal = net.ReadVector()

		ParticleEffect("pickaxe_hit01", pos, normal:Angle())
	end)
