...
function HEALTH:RemoveHediffByID(id)
	...
	if found then
		...
		if SERVER then
			self:OnUpdateDiffs()
			self:GetBleedRate() -- immediately update isBleeding NetVar on removal too
...
function HEALTH:AddHediff(uniqueID, hitGroup, data, id)
	...
	if SERVER then
		...
		self:OnUpdateDiffs()
		self:GetBleedRate() -- immediately update isBleeding NetVar without waiting for Tick
