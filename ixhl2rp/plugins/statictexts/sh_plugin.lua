...
ix.config.Add("annotationDuration", 240, "@annotationDurationDesc", nil, {
...
do
	local CMD = {}
	CMD.description = "@cmdSceneTextDesc"
	...
	function CMD:OnRun(client, annotationText)
		...
		client:NotifyLocalized("annotation.placed")
...
do
	local CMD = {}
	CMD.description = "@cmdSceneTextRemoveDesc"
	CMD.privilege = "Map Annotations"
		...
		for index, annotation in ipairs(annotations) do
			if annotation.position:DistToSqr(hitPosition) < 2500 then
				if annotation.ownerSteamID == client:SteamID() or CAMI.PlayerHasAccess(client, "Helix - Map Annotations") then
					...
					client:NotifyLocalized("annotation.removed")
					ix.log.Add(client, "annotationDeleted", annotation.content)
				else
					client:NotifyLocalized("annotation.noPermission")
				end
				return
			end
		end

		client:NotifyLocalized("annotation.notFound")
	end
