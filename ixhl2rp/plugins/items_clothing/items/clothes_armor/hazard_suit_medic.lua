-- Медицинский костюм химзащиты: полностью заменяет модель персонажа на защитную
-- (отдельные модели для мужского и женского пола). Даёт сопротивление
-- радиации/токсинам (rad_resist), учитывается системой радиации (GetRadResistance
-- суммирует rad_resist предметов из char_outfit.armor).
--
...
ITEM.name = "Медицинский костюм химзащиты"
ITEM.description = "Герметичный медицинский защитный костюм. Полностью укрывает тело от заражённой среды и токсичной флоры. Максимальная защита — вместе с противогазом и фильтром."
