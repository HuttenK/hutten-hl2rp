	...
	function PLUGIN:LoadData()
		...
		if data then
			for _, v in ipairs(data) do
				local data2 = ix.container.stored[v[3]:lower()]
				...
				if data2 then
					...
					entity:CreateInventory(data2)

					if v[7] then
						entity:LoadItems(v[7])
					end
...
function PLUGIN:InitializedPlugins()
	for k, v in pairs(ix.container.stored) do
		if !v.name or !v.width or !v.height then
...
properties.Add("container_setpassword", {
	...
	Receive = function(self, length, client)
		...
		ix.log.Add(client, "containerPassword", name, inventory.id, password:len() != 0)
	end
})
...
properties.Add("container_setname", {
	...
	Receive = function(self, length, client)
		...
		ix.log.Add(client, "containerName", name, inventory.id, name:len() != 0)
	end
})