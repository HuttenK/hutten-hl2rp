...
if (CLIENT) then
	...
	function PLUGIN:CreateChat()
		...
		local tabs = ix.option.Get("chatTabs", "")
		local pos = ix.option.Get("chatPosition", "")

		self.panel = vgui.Create("ixChatbox")
		self.panel:SetupTabs(istable(tabs) and tabs or util.JSONToTable(tabs))
		self.panel:SetupPosition(istable(pos) and pos or util.JSONToTable(pos))
