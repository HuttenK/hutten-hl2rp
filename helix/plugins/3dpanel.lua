...
if (SERVER) then
	...
	function PLUGIN:AddPanel(position, angles, url, scale, brightness, w, h)
		w = w or 0
		h = h or 0
	...
	local function CacheMaterial(index)
		...
		local fixDimension = (!info[3] and !info[4]) or (info[3] == 0 or info[4] == 0)
	...
	net.Receive("ixPanelAdd", function()
		...
		if (url != "") then
			if (w and h and (w == 0 or h == 0)) then
				w = nil
				h = nil
			end
	...
	function PLUGIN:PreviewPanel()
		...
		if (cachedPreview[2] and !cachedPreview[2]:IsError()) then
			...
			local modWidth, modHeight = tonumber(arguments[4]) or 0, tonumber(arguments[5]) or 0
			...
			if modWidth > 0 and modHeight > 0 then
				width = modWidth
				height = modHeight
			end
...
ix.command.Add("PanelAdd", {
	...
	OnRun = function(self, client, url, scale, brightness, width, height)
		...
		-- Add the panel.
		PLUGIN:AddPanel(position + angles:Up() * 0.1, angles, url, scale, brightness, width, height)
		return "@panelAdded"
	end
})
...
ix.command.Add("PanelInfo", {
	...
	OnRun = function(self, client, radius)
		...
		for k, v in pairs(PLUGIN.list) do
			...
			if v[1]:Distance(position) <= radius then
				client:ChatNotify(string.format("URL: %s | Scale: %f | Brightness: %f | Dimensions: %f %f", v[6] or "", (v[5] or 0.1) * 10, v[8] or 0, v[3] or 0, v[4] or 0))
			end
		end
