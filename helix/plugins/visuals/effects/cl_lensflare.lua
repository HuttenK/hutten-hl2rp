...
local d1, d2 = scrW / 2, scrH / 2

hook.Add("OnScreenSizeChanged", "autonomous.lensflare", function()
	scrW, scrH = ScrW(), ScrH()
	d1, d2 = scrW / 2, scrH / 2
end)
...
local function DrawFlare()	
	...
	if !sun or sun.obstruction == 0 or !sun.obstruction then return end
	

	local dir = sun.direction or vector_origin
	local sunpos = (EyePos() + dir * 4096):ToScreen()
	local dot = (dir:Dot(EyeVector()) - 0.8) * 5
	local rSz = scrW * 0.15

	local aMul = math.Clamp((dir:Dot(EyeVector()) - 0.4) * (1 - math.pow(1 - sun.obstruction,2)),0,1)
