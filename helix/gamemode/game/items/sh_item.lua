...
if SERVER then
	...
	net.Receive('item.legacy.stack.combine', function(len, client)
		...
		-- Calculate final values before any SetData to avoid reading stale state
		local newItemValue   = value - sentStacks
		local newTargetValue = targetValue + sentStacks

		targetItem:SetData("stack", newTargetValue)

		if newItemValue <= 0 then
			item:Remove()
		else
			item:SetData("stack", newItemValue)
		end
	end)
