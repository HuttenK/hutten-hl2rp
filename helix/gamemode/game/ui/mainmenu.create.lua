...
local radial_clr = Color(55, 10, 10)
...
function PANEL:CreateFactionStage()
	...
	title:SetText(L("charcreate_faction"))
	...
	factionName:SetText(L("charcreate_citizen"))
	...
	local function UpdateFactionInfo(info)
		...
		factionName:SetText(L(info.name):utf8upper())
		factionName:SizeToContents()
		factionName:CenterHorizontal()

		self.factionPanelInfo.text = ix.markup.Parse([[<font=char.create.factionDesc><color=150,214,248>]]..(info.info and L(info.info) or "")..[[</color></font>]], maxDescWidth * 0.9)
	end
	...
	--for i = 1, #factionsTest do
		...
		panel.hoverClr = disable and Color(40, 5, 5) or Color(80, 0, 0)
...
function PANEL:CreateVisualStage(container)
	...
	title:SetTextColor(self.clr.title)
	title:SetContentAlignment(6)
	title:SetText(L("charcreate_visual"))
	title:SizeToContents()
	title:AlignRight(padding)
	title:AlignTop(padding)
	...
	self:CreateContainer(1, L"charcreate_phys", function(container)
		...
		eyes:SetValue("")
		eyes:SetValueColor(Color(200, 64, 64))
...
function PANEL:CreateSkillsStage()
	...
	title:SetTextColor(self.clr.title)
	title:SetContentAlignment(6)
	title:SetText(L("charcreate_attributes"))
	title:SizeToContents()
	title:AlignRight(padding)
	title:AlignTop(padding)
	...
	focusFrame:AddLabel(L("charcreate_choose"), "char.create.button", ix.Palette.combineblue)
	local PrimaryLabel = focusFrame:AddLabel("", "attribute.maxpoints", ix.Palette.combineyellow)
	local primaryAtt = focusFrame:AddLabel(L("charcreate_primaryAttributes"), "char.create.button", ix.Palette.combineyellow)
	primaryAtt:SetAutonomousTooltip(function(tooltip)
		tooltip:SetTitle(L("charcreate_primaryAttribute"))
		tooltip:AddSmallText(L("charcreate_primaryAttributeDesc"))
		tooltip:AddDivider()

		tooltip:AddMarkup([[<font=autonomous.hint.info><colour=210,240,250>]] .. L("charcreate_primaryAttributeHint") .. [[</colour></font>]])		
	end)
...
function PANEL:Paint(w, h)
	surface.SetDrawColor(32, 8, 8, 255 * 0.9)
	surface.DrawRect(0, 0, w, h)

	surface.SetDrawColor(32, 8, 8)
