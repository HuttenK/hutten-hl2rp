...
local styles = {
	[1] = {
		outline = Color(248, 64, 64, 48),
		outline2 = Color(248, 64, 64, 16),
		corners = Color(248, 64, 64, 255),
		text = Color(248, 64, 64, 255),
...
local function DrawCorners(x, y, w, h, z)
	surface.SetDrawColor(48, 8, 8, 128)
	surface.DrawRect(x, y, w, h)

	local size = ix.UI.Scale( (h / 4) * 0.1 )

	surface.SetDrawColor(248, 64, 64, 255)
	surface.DrawLine(x, y, x + size, y)
	surface.DrawLine(x, y, x, y + size)
	...
	surface.SetDrawColor(248, 64, 64, 255 * 0.25)
...
function PANEL:Init()
	...
	self:AddTab(info, function(self, container)
		...
		label:SetTextColor(Color(248, 64, 64))
		...
		local function AddLabel(text1, text2)
			local label = left_container:Add("DLabel")
			label:SetTextColor(Color(248, 64, 64, 255))
			label:SetText(text1)
		...
		local function AddTextBox(value, minHeight, onClick)
			...
			textbox.Paint = function(this, width, height)
				surface.SetDrawColor(225, 64, 64, 16)
	...
	self:AddTab(skills, function(self, container)
		...
		for k, v in pairs(ix.specials.list) do
			...
			attribute:Dock(TOP)
			attribute:SetTextColor(color_white, Color(248, 64, 64))
		...
		for k, v in SortedPairs(categories) do
			...
			attribute:DockMargin(32, 0, 32, 5)
			attribute:Dock(TOP)
			attribute:SetTextColor(color_white, value > 0 and Color(248, 64, 64) or Color(128, 128, 128))
			attribute:SetFont("ui.specialpoints.title", "ui.specialpoints.title")
...
function PANEL:PaintPanel(w, h)
	surface.SetDrawColor(48, 8, 8, 100)
	surface.DrawRect(0, 0, w, h)

	DisableClipping(true)
		surface.SetDrawColor(248 * 0.5, 64 * 0.5, 64 * 0.5, 96)
...
function PANEL:Init()
	...
	self.tab.Paint = function(this, w, h)
		-- Guard: plugin or character may be unavailable
		if not LEVEL_PLUGIN then return end
		local character = LocalPlayer():GetCharacter()
		...
		-- Guard: level 100 has no XP table entry (returns 0) — clamp to avoid div/0
		...
		draw.SimpleText(tostring(level), "ui.tabmenu.level", x, 0, level_clr, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
		...
		DisableClipping(true)
			...
			draw.SimpleText(L("tab_xp_until", xp_remaining), "ui.tabmenu.levelmini", x + level_bar_w, y, level_clr, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
		DisableClipping(false)
	end

	local credits_clr = Color(248, 64, 64, 8)
	...
	self.time:SetTextColor(Color(248, 64, 64, 64))
...
function PANEL:Think()
	...
	if character and (!self.nextSlowThink or self.nextSlowThink <= CurTime()) then
	...
	function PANEL:Draw3DCursor()
		...
		cam.Start3D2D(pos, ang, math.pow(trace.Fraction, .1) * (a or .2))
			cam.IgnoreZ(true)
				...
				Circle(0, 0, 8, 18, Color(248, 64, 64), 0)
