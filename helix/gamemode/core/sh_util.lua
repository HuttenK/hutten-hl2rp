...
local function SoundDurationLinux(soundPath)
	...
	if extension and soundDecoders[extension] then
		...
		-- Файла может не быть на диске (кастомная озвучка, префикс ../../hl2/sound/,
		-- звук внутри VPK/GMA) — file.Open вернёт nil. Декодеры (wav/mp3) индексируют
		-- buffer без проверки и роняли сервер. Откатываемся на движковую длительность.
		if not buffer then
...
function ix.util.Stutter(phrase, stamina_loss)
	...
	for i = 1, #split_phrase do
		...
		if string.utf8lower(first_sound) == "c" or string.utf8lower(first_sound) == "�" or string.utf8lower(first_sound) == "�" then
